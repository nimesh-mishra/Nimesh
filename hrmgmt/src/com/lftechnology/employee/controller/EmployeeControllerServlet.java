...
public class EmployeeControllerServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		String pageType=(String) request.getParameter(SEARCH);
	...
	protected void doAdd(HttpServletRequest request,
		...
		if (success) {
			...
			employee = employeeAction.getEmployeeObject(request, response);
			boolean success=employeeServiceImpl.updateEmployeeDetail(employee, empId);
			try {
				response.sendRedirect(VIEWEMPLOYEE);
			} catch (IOException e) {
				LOG.severe("Error Occurred while dispatching request");
	...
	protected void doView(HttpServletRequest request,
			HttpServletResponse response) {
		int deleteId=Integer.parseInt(request.getParameter("deleteButtonId"));
		int editId=Integer.parseInt(request.getParameter("editButtonId"));
		if(!(deleteId==0)&&(editId==0)){
			...
			RequestDispatcher dispatch=request.getRequestDispatcher(response.encodeURL(ADDEMPLOYEE));
			 try {
				dispatch.forward(request, response);
	...
	protected void doSearch(HttpServletRequest request,
		...
		if(employeeList==null){
			...
			try {
				RequestDispatcher dispatch=request.getRequestDispatcher(response.encodeURL(SEARCHEMPLOYEE));
				 dispatch.forward(request, response);
		...
		// if filter is applied and search is not.Filters data in already searched result
		if (((filterType != null) || !(filterType.equals("")))
				&& ((searchText == null || searchText.equals("")||searchText.contains("Enter")))) {
